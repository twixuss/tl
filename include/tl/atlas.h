...
// TODO: EmptyAtlasTraits

template <class Element, class Allocator = Allocator>
struct DefaultAtlasTraits {
	struct AddElementsInfo {
		Element *elements;
		v2umm size;
		// stride_in_bytes - if 0, replaced with `info.size.x * sizeof(Element)`
		umm stride_in_bytes;
	};
	...
	void _write(v2u cursor, AddElementsInfo info) {
		u32 &stride = info.stride_in_bytes;
		if (stride == 0)
			stride = info.size.x * sizeof(Element);

		for (u32 y = 0; y < info.size.y; ++y) {
			auto dst = &data[(cursor.y + y) * size.x + cursor.x];
			auto src = (u8 *)new_elem + y * stride;
			memcpy(dst, src, info.size.x * sizeof(Element));
		}
	}

	void _grow(v2u &size, v2u &cursor) {
		// Double size in one dimension, keep old stuff where it is,
		// new elements will be placed in new space
	...
	void _pack_end(Element *old_data) {
		allocator.free(old_data);
	}
};

template <class Area, class Traits>
	requires requires(Traits traits) {
		{ traits.size } -> std::same_as<v2u>;
	}
struct Atlas : Traits {
	List<Area, Allocator> areas;
	...
	void add(Area area, AddElementsInfo info) {
		...
		while (1) {
			if (cursor.x + info.size.x <= size.x && cursor.y + info.size.y <= size.y)
				break;
		...
		this->_write(cursor, info);

		area.rect = {cursor, cursor + info.size};
		areas.add(area);
		
		on_modify(this, user_data, area.rect);

		cursor.x += info.size.x;
		row_height = max(row_height, info.size.y);
	}

	void grow() {
		this->_grow(size, cursor);
	}

	void pack(bool keep_area_order = false) {
		auto ctx = this->_pack_begin();

		auto old_areas = areas;
		defer { free(old_areas); };

		areas = {};
		areas.reserve(old_areas.count);
		...
		for (auto old_area : old_areas) {
			...
			add(old_area, {
				.elements = elem,
				.size = old_area.rect.size(),
				.stride_in_bytes = size.x * sizeof(Element),
			});
		}
