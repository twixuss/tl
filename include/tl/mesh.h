...
TL_API CommonMesh parse_glb_from_memory(Span<u8> memory);

inline Optional<CommonMesh> parse_glb_from_file(Span<filechar> path) {
...
CommonMesh parse_glb_from_memory(Span<u8> memory) {
	using namespace Glb;
	...
	if (normal_attribute_index) {
		...
		if (normal_count != position_count) {
			print("Failed parsing glb file. Normal count does not match position count.\n");
			return {};
		}
	}
	...
	if (color_attribute_index) {
		...
		if (color_count != position_count) {
			print("Failed parsing glb file. Color count does not match position count.\n");
			return {};
		}
